...
mkdir $HOME/conserved_gene_order_demo
cd $HOME/conserved_gene_order_demo
...
cd $HOME/conserved_gene_order_demo/ncbi-blast-2.11.0+/bin
...
cd ..
mkdir $HOME/conserved_gene_order_demo/blast_db
export BLASTDB=$HOME/conserved_gene_order_demo/blast_db
set BLASTDB=$HOME/conserved_gene_order_demo/blast_db
cd $HOME/conserved_gene_order_demo/blast_db


# Make the blast database.

makeblastdb -in $HOME/conserved_gene_order_demo/human_reference/human_reference_clean.fa -dbtype prot -parse_seqids -out database_human
cd $HOME/conserved_gene_order_demo/human_reference


# Run blastp.

blastp -num_threads 16 -db $HOME/conserved_gene_order_demo/blast_db/database_human -evalue 1e-10 -outfmt "6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore slen qlen" -qcov_hsp_perc 80 -query $HOME/conserved_gene_order_demo/human_reference/human_reference_clean.fa >'results_human.txt'
cd $HOME/conserved_gene_order_demo/human_reference
...
mkdir $HOME/conserved_gene_order_demo/macaca_reference
cd $HOME/conserved_gene_order_demo/macaca_reference
...
rm macaca_reference_cln.fa                                                                              demo_blast_script.sh
...
cd $HOME/conserved_gene_order_demo/blast_db
makeblastdb -in $HOME/conserved_gene_order_demo/macaca_reference/macaca_reference_clean.fa -dbtype prot -parse_seqids -out database_macaca
cd $HOME/conserved_gene_order_demo/macaca_reference


# Run blastp. Human - Macaca

blastp -num_threads 16 -db $HOME/conserved_gene_order_demo/blast_db/database_macaca -evalue 1e-10 -outfmt "6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore slen qlen" -qcov_hsp_perc 80 -query $HOME/conserved_gene_order_demo/human_reference/human_reference_clean.fa >'results_human-macaca.txt'
